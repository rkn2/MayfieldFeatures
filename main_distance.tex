...
Figure~\ref{fig:perm_imp} presents the permutation importance rankings for the Random Forest model. To evaluate the stability of these rankings, pairwise Wilcoxon signed-rank tests were conducted between adjacent features. The analysis revealed two distinct statistical tiers ($p < 0.05$). The first tier consists solely of \textbf{Distance}, confirming it as the dominant predictor statistically superior to all intrinsic attributes. The second tier comprises a cluster of structural features---including \textbf{Roof Substrate}, \textbf{Wall Thickness}, \textbf{Parapet Height}, and \textbf{Wall Slenderness}---which, while offering significant predictive power over random noise, are statistically indistinguishable from each other ($p > 0.05$ for pairwise comparisons). This indicates that rather than a strict hierarchy of defects, vulnerability is driven by a broad constellation of envelope and structural deficiencies acting in concert. The high importance of missingness-sensitive features like wall thickness (Appendix~\ref{sec:sensitivity}) suggests this tier also captures informative data gaps.
