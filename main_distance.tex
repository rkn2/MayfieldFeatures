...
Key building features identified include retrofit type (the presence and type of structural retrofits) and wall cladding (the material and condition of the exterior skin), which act as critical modifiers of vulnerability. Additionally, roof substrate (the material decking beneath the roof covering) and roof slope (the geometry influencing aerodynamic uplift) emerged as significant predictors, underscoring the importance of the building envelope's integrity and aerodynamic profile in tornado resistance.
...
SHAP interaction analysis revealed patterns where hazard exposure amplifies specific building vulnerabilities. For Class 2 (Significant Damage), the strongest interactions involved distance combined with occupancy and wall substrate, reflecting how proximity to the tornado compounds risks inherent to specific building functions and material systems. Table~\ref{tab:shap_interactions} lists the top feature interactions. Notably, purely structural interactions such as foundation type $\times$ retrofit type also emerge as top drivers, suggesting that the effectiveness of retrofits is heavily dependent on the foundation system's ability to transfer loads.
...
\begin{tabular}{lc}
\toprule
\textbf{Feature Interaction} & \textbf{Strength} \\
\midrule
Distance $\times$ Occupancy & 0.0125 \\
Occupancy $\times$ Wall Substrate & 0.0121 \\
Distance $\times$ Wall Substrate & 0.0092 \\
Foundation $\times$ Occupancy & 0.0086 \\
Foundation $\times$ Retrofit Type & 0.0085 \\
Roof Shape $\times$ Wall Substrate & 0.0079 \\
Distance $\times$ Foundation & 0.0078 \\
Occupancy $\times$ Retrofit Type & 0.0078 \\
Foundation $\times$ Wall Cladding & 0.0073 \\
Foundation $\times$ Wall Substrate & 0.0072 \\
\bottomrule
\end{tabular}
\end{table}
...
\subsection{Sensitivity Analysis: Impact of Missing Data}
Given the high missingness in features such as wall thickness (36\%) and foundation type (36\%), a sensitivity analysis was conducted to evaluate the robustness of these findings under different imputation strategies. We compared the baseline Median/Informative imputation against Multiple Imputation by Chained Equations (MICE) and Missingness Indicators.
