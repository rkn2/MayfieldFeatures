...
A synthetic random feature is introduced as a negative control. If this noise variable appears among the important features, the analysis validity is questionable \cite{merrick2019randomized}. This provides an objective quality check that is largely absent from existing disaster assessment studies.
...
All the models employed balanced class weighting to address class imbalance, while hyperparameters were selected based on preliminary tuning (Table~\ref{tab:hyperparameters} in Appendix~\ref{sec:hyperparameters}).
...
Figure~\ref{fig:perm_imp} presents the permutation importance rankings for the Random Forest model. The results show that distance from the tornado path is the dominant predictor of damage. This validates the physical reality that proximity to the vortex core outweighs most structural differences. For preservation applications, however, several building features rank above the random noise threshold, indicating they offer statistically significant modifications to vulnerability even after accounting for distance.

Figure~\ref{fig:perm_imp} presents the permutation importance rankings for the Random Forest model. To evaluate the stability of these rankings, pairwise Wilcoxon signed-rank tests were conducted between adjacent features. The analysis revealed two distinct statistical tiers ($p < 0.05$). The first tier consists solely of Distance, confirming it as the dominant predictor statistically superior to all intrinsic attributes. The second tier comprises a cluster of structural features---including Roof Substrate, Wall Thickness, Parapet Height, and Wall Slenderness---which, while offering significant predictive power over random noise, are statistically indistinguishable from each other ($p > 0.05$ for pairwise comparisons). This indicates that rather than a strict hierarchy of defects, vulnerability is driven by a broad constellation of envelope and structural deficiencies acting in concert. The high importance of missingness-sensitive features like wall thickness (Appendix~\ref{sec:sensitivity}) suggests this tier also captures informative data gaps.

\begin{figure}[h!]
    \centering
    \includegraphics[width=1\textwidth]{tornado_vulnerability_outputs_engineered/delta_accuracy_engineered.png}
...
The inclusion of engineered geometric ratios provided additional physical insight, though raw dimensions remained highly predictive. Wall Slenderness emerged as a top-tier predictor (Rank 5), confirming that the relationship between height and thickness drives vulnerability. Raw Wall Thickness (Rank 3) performed comparably to the slenderness ratio within this statistical tier, suggesting that absolute material depth---which correlates with shear capacity and debris resistance---is as critical as stability against out-of-plane buckling. Parapet Slenderness (Rank 8) also proved significant, validating the hypothesis that thin, cantilevered elements at the roofline are specific points of failure. Mean Fenestration (Rank 13) contributed moderately, supporting the "Swiss cheese" vulnerability theory, but was secondary to primary structural system indicators.
...
Flat roofs ranked among top predictors, consistent with wind tunnel measurements showing higher suction versus gabled roofs \cite{razavi2021effects,Kopp2005}. However, attributing this solely to static corner suction is insufficient, as tornado vortices differ fundamentally from boundary layer winds: they exhibit highly three-dimensional flow with significant vertical velocity components and create time-dependent loading from the translating vortex. Consequently, suction peaks may occur at different locations than those predicted by standard Atmospheric Boundary Layer (ABL) flow. It is not yet possible to distinguish whether failures stem from excessive uplift forces or inadequate connections. CFD modeling of tornado vortices on buildings with varied roof slopes could quantify these unique pressure distributions, while experimental testing must move beyond quasi-steady design assumptions to capture transient tornado effects.
...
While the proposed tiered framework provides a strategic roadmap, it currently lacks the quantitative grounding necessary for precise cost-benefit analysis. Future work must bridge this gap by establishing typical retrofit costs per building, using data from National Park Service guidance or industry standards to move the framework from aspirational to operational. Future methodology should also explicitly correlate predicted damage classes with observed failure modes from detailed field reports (FAST data); while statistical interactions imply load path dependencies, distinguishing between connection failure, member failure, and progressive collapse is required to confirm failure sequences (e.g., identifying if roof loss occurred with intact walls versus wall collapse triggering roof failure). Additionally, finite element modeling (FEM) is required to quantify how much specific interventions, such as parapet bracing, reduced failure probability under varying wind loads, following FEM frameworks established for masonry systems \cite{masoomi2018}. Finally, a decision-support tool should be developed to help communities prioritize interventions within fixed budgets, translating these technical findings into actionable policy.
...
The Random Forest model and SHAP analysis provide a data-driven justification for prioritizing roof-to-wall connections and envelope integrity in preservation retrofits. Instead of supporting blanket condemnation of URM structures, this work supports a tiered, risk-informed framework for preservation. By targeting interventions—such as parapet bracing and chimney anchorage—to the "transition zone" where building performance determines survival, preservation authorities can balance structural safety with the stewardship of cultural heritage.
...
Table~\ref{tab:hyperparameters} details the specific hyperparameter configurations used for the final model training, selected based on preliminary tuning to balance performance and generalizability.
...
\midrule
Random Forest & n\_estimators=200, min\_samples\_leaf=2, class\_weight='balanced\_subsample' \\
XGBoost & n\_estimators=300, max\_depth=6, learning\_rate=0.08, subsample=0.8, colsample\_bytree=0.8, objective='multi:softprob' \\
Logistic Regression & class\_weight='balanced', max\_iter=1000 \\
Ridge Classifier & class\_weight='balanced' \\
Linear SVC & class\_weight='balanced' \\
Decision Tree & Default parameters (random\_state=42) \\
\bottomrule
\end{tabular}
\end{table}
...
The results confirm that distance to tornado path remains the dominant predictor (Rank 1) across all missingness assumptions, demonstrating the robustness of the hazard-focused model. However, the importance of wall thickness drops significantly under MICE (Rank 16 vs. Rank 5 in baseline), suggesting that its predictive power is partially derived from ``informative missingness''---i.e., the data is missing because the walls collapsed or were inaccessible due to severe damage. Conversely, foundation type rises in importance under MICE (Rank 5 vs. Rank 27), indicating that if foundation details were effectively recovered, they would likely serve as critical predictors of survival. These findings reinforce the tiered framework: while global hazard parameters are robust, specific structural attributes like wall thickness should be interpreted with the caveat that their signals may reflect damage-induced unobservability.
